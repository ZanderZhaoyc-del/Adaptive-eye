...
5. If `screenshotPath` exists, use it for visual contrast fallback when the user wants vision-based analysis.
6. If `warnings` is non-empty, include those warnings without treating the entire audit as failed.
7. If `annotatedScreenshotPath` exists, include the annotated screenshot path in the response.
...
- Multi-page crawling
- Vision-language-model fallback execution
